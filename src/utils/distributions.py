...
class analyticalImplementation:
    """
    Analytical implimentations of the various distribtuion functions.
    """
    ...
    def two_signal_pdf(self, M, f1, f2, lam, mu1, mu2, sg):
        return (
            f1 * self.signal_pdf_trunc(M, mu1, sg)
            + f2 * self.signal_pdf_trunc(M, mu2, sg)
            + (1 - f1 - f2) * self.background_pdf_trunc(M, lam)
        )
...
class numbaImplementation:
    ...
    # Inverse CDFs for generating samples

    def background_inv_cdf(self, p, lam):
        return truncexpon.ppf(p, xmin=self.a, xmax=self.b, loc=self.a, scale=1 / lam)

    def signal_inv_cdf(self, p, mu, sg):
        return truncnorm.ppf(p, xmin=self.a, xmax=self.b, loc=mu, scale=sg)
