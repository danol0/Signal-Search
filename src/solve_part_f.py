...
def single_peak_test(sample, binned=False):
    ...
    Methodology is discussed in section 3.2.1 of the report.
    ...
    # H1: f != 0
    mi_h1 = Minuit(nll, f=0.1, lam=mi_h0.values["lam"], mu=5.3, sg=0.02)
...
plt.savefig('report/figures/part_f_H0_distribution.png')
...
plt.xlim(0, 100)
plt.legend()
plt.savefig('report/figures/part_f_H1_distribution.png')
...
popt, pcov = curve_fit(sigmoid, sample_sizes, y, p0=[0.5, 0.01, 500, 0.5], method="lm", sigma=y_e)
...
plt.savefig('report/figures/part_f_power.png')
